...
<div style="white-space: preserve nowrap; font-size: 85%; margin-left: 50px;"> typedef struct { 
...
‎     GLenum draw_mode;     ‎ // If wire_frame, then edges are drawn, if Full, then triangles are drawn. 
  } Rendered_Object;
‎ 
Rendered_Object* scene_1(int* total_objs, Mesh*** mesh_lst, int* num_meshes, Material*** mat_lst, int* num_mats){
‎   static Transform walls = { .actions=NULL, .position={0, 0, 0},  .rotation={0, 0, 0}, .scale={1, 1, 1} }; //defining transform 
‎  ...
‎   static Material* materials[] = { &Base_Color }; //defining shader
‎   ...
‎ static Rendered_Object scene[] = {
  ‎ ‎         { .parent_mesh = &Room_Mesh,      .material = &Base_Color, .transform = &walls, .draw_mode = GL_TRIANGLES }, //creating one rendered object. 
